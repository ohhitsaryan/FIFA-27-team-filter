...
* **Performance:** Optimized to maintain a stable 60 FPS render loop using GPU acceleration via MediaPipe WebAssembly and HTML5 Canvas layers.
* **Usability:** Responsive, touch-friendly UI design featuring intuitive card layouts, real-time feedback badges, and automated state-machine transitions (`loading` → `waiting` → `ready` → `spinning` → `result`).
...
src/
  ...
  style.css             # Stitch "FIFA Broadcast — Dark Pitch" design system
scripts/
...
The visual language is **FIFA Broadcast — Dark Pitch**, synthesized from Stitch mockups:
...
Private project. Team crests in `public/logos/` are © their respective clubs and used here under FIFA's licensed club artwork program.
